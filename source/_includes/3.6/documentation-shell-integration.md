...
<a id="opt_xonsh" style="cursor:pointer;" onclick="show('xonsh')">xonsh</a> |
...
<pre>mkdir -p ~/.config/xonsh/rc.d
curl -L https://iterm2.com/shell_integration/xonsh \
-o ~/.config/xonsh/rc.d/iterm2.xsh</pre>

Xonsh automatically loads scripts from <code>~/.config/xonsh/rc.d/</code>, so no additional configuration is needed. If you use a custom <code>$XDG_CONFIG_HOME</code>, adjust the path accordingly.
</div>
