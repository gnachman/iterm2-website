...
#
# This is a port of FinalTerm's bash_startup.in with minor changes for Mac
# compatibility.
# - Termlet support removed
# - Terminal commands removed
# - preexec.bash inlined

## Begin preexec.bash (inlined)
...
# TODO: This appears to break if there is already a DEBUG trap installed
...
function remote_host_sequence() {
	printf "\033]1337;RemoteHost=%s@%s\007" "$USER" $(hostname -f)
}

function current_dir_sequence() {
        printf "\033]1337;CurrentDir=%s\007" "$PWD"
}

# Send sequence marking the start of a command
PS1="$PS1\[$(final_term_control_sequence 'B')$(remote_host_sequence)$(current_dir_sequence)\]"
